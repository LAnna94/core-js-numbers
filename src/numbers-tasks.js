...
function toNumber(value, def) {
  const result = Number.isNaN(Number(value)) ? def : Number(value);
...
function toPrecision(number, precision) {
  ...
  if (precision - str.length > 0) {
    return number.toFixed(precision - str.length).toString();
  }
  return number.toFixed(str.length - precision).toString();
}
...
function getSumOfNumbers(x1, x2, x3) {
  const res = x1 + x2 + x3;
  const formatedNumber = Number.isInteger(res)
    ? res
    : Number.parseFloat(res).toFixed(1);
